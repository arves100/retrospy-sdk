...
static int SendChallengeResponse(const char *indata, int _gameport);
...
int InitStatsConnection(int _gameport)
{
	int aResult = InitStatsAsync(_gameport, 0);
...
static int SendChallengeResponse(const char *indata, int _gameport)
{
	...
	DOXCODE(respformat, sizeof(respformat)-1, enc3);
	len = sprintf(resp,respformat,gcd_gamename, md5val, _gameport);
